<template>
  <section class="review-section">
    ...
    <div class="review-info">
      <p>共审查了 <span>{{ reviewResults.length }}</span> / {{ totalRequirements }} 条需求</p>
    </div>
    
    <div v-if="!reviewStarted" class="start-review-container">
  <button class="start-review-btn" @click="handleStartReview" :disabled="loading">
    <span v-if="!loading">开始审查</span>
...
<div v-else-if="loading" class="loading-container">
    ...
    <StatusMessage 
      ...
      :type="statusType" 
    />
    <div v-if="reviewStarted && !loading && reviewResults.length === totalRequirements" class="next-step-container">
      <button class="next-step-btn" @click="$emit('review-completed', reviewResults)">下一步</button>
    </div>
  </section>
</template>
...
const currentRequirementIndex = ref(0);
const progressPercentage = ref(0);
const currentStatusText = ref('准备开始AI文档审查...');
